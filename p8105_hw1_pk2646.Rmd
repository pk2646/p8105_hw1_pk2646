...
The penguins dataset has the the following variable names - `r names(penguins)`. The species are `r levels(penguins$species)` and the islands are namely `r levels(penguins$island)`. The average bill length is `r mean(penguins$bill_length_mm, na.rm = TRUE)` and bill depth is `r mean(penguins$bill_depth_mm, na.rm = TRUE)`. The average body mass is `r mean(penguins$body_mass_g, na.rm = TRUE)`. The dataset has `r ncol(penguins)` variables and `r nrow(penguins)` observations. The mean flipper length is `r mean(penguins$flipper_length_mm, na.rm = TRUE)`. 
